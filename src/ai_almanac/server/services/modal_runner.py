...
from ai_almanac.server.services.bucket_mounts import (
    outputs_bucket_name,
    translate_job_config,
)
...
def _is_stageable(uri: object) -> bool:
    ...
    volume. A relative or empty ref is unstageable.
    """
    text = str(uri)
...
def _preflight_error(config: dict, outputs_bucket: str) -> str | None:
    ...
    Called on the already-translated config (mount paths converted to gs://
    URIs by translate_job_config). An outputs bucket must be configured and all
    input paths must be gs:// URIs or absolute mount paths.
    """
    if not outputs_bucket:
        return (
            "job_runner=modal requires the outputs dir to be mapped in bucket_mounts "
            "as a bare-bucket gs:// URI."
        )

    # Forecast jobs run live inference against GFS — there is no obs_dir/model_dir
    # on the forecast job's own config. The live-scoring step stages historical
    # data from the parent blend's frozen config snapshot.
    ...
    if dataset_config.get("provider") not in _REMOTE_OBS_PROVIDERS:
        ...
        if not str(obs_dir).startswith("gs://"):
            return (
                "job_runner=modal requires obs_dir to be a gs:// URI or a path "
                f"under bucket_mounts; got {obs_dir!r}."
            )

    # Blend jobs (and forecast jobs' blend snapshot) carry per-model {year}.nc
    # staging URIs; benchmark jobs stage a single model dir.
    ...
    if not str(model_dir).startswith("gs://"):
        return (
            "job_runner=modal requires model_dir to be a gs:// URI or a path "
            f"under bucket_mounts; got {model_dir!r}."
        )
...
class ModalRunner:
    ...
    async def submit(self, request: ExecutionRequest) -> RunnerHandle:
        raw_config = await _job_config(request.job_id)
        # Translate mount paths to gs:// URIs for Modal workers. The stored
        # config stays in mount-path form; the translated copy is dispatch-only.
        translated = translate_job_config(raw_config)
        bucket = outputs_bucket_name()
        error = _preflight_error(translated, bucket or "")
        if error:
            raise ModalPreflightError(error)

        # The Modal SDK calls block on the network; keep them off the event loop.
        call_id = await asyncio.to_thread(self._spawn, request.job_id, translated, bucket)
    ...
    def _spawn(self, job_id: str, config: dict, outputs_bucket: str | None) -> str:
        import modal
        ...
        call = function.spawn(job_id, config, outputs_bucket)
