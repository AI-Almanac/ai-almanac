...
@pytest.mark.asyncio
async def test_private_job_hidden_from_other_users(
...
@pytest.mark.asyncio
async def test_owner_reads_own_private_job(
    client: httpx.AsyncClient, proxy
) -> None:
...
@pytest.mark.asyncio
async def test_shared_job_readable_by_other_users(
    client: httpx.AsyncClient, proxy
) -> None:
...
@pytest.mark.asyncio
async def test_admin_reads_any_private_job(
    client: httpx.AsyncClient, proxy
) -> None:
    job_id, _owner = await _owned_job(visibility="private")
...
async def test_owner_can_share_and_unshare(
    ...
    shared = await client.post(
        f"/jobs/{job_id}/share", headers={"X-Forwarded-User": owner}
    )
    assert shared.status_code == 200
    assert shared.json()["visibility"] == "shared"

    unshared = await client.post(
        f"/jobs/{job_id}/unshare", headers={"X-Forwarded-User": owner}
    )
...
async def test_non_owner_cannot_share(client: httpx.AsyncClient, proxy) -> None:
    ...
    resp = await client.post(
        f"/jobs/{job_id}/share", headers={"X-Forwarded-User": "intruder"}
    )
    assert resp.status_code == 404


@pytest.mark.asyncio
async def test_sharing_is_read_only_no_cancel_or_delete(
    client: httpx.AsyncClient, proxy
) -> None:
    job_id, _owner = await _owned_job(visibility="shared", status="running")
    cancel = await client.post(
        f"/jobs/{job_id}/cancel", headers={"X-Forwarded-User": "reader"}
    )
